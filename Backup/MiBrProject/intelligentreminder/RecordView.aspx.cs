...
public partial class RecordView : InternetWebPage {
    public static string FD = "^~^";
    public RecordView() {
        Load += new EventHandler(RecordView_Load);
    }
   ...
   //    Response.Write("|" + str + ":" + obj.ToString());
    ...
    void RecordView_Load(object sender, EventArgs e) {
        DateTime rightNow = DateTime.Now;
        StreamReader sr = null;
        int cnt = 0;
        try {
            sr = new StreamReader(Request.InputStream);
            string phoneid = getMeMyNextField(sr);
            string myAddNbr = getMeMyNextField(sr);

            List<DbParameter> parms = new List<DbParameter>();
            parms.Add(CommonMethods.buildStringParm("phoneidparm", phoneid, ParameterDirection.Input));
            parms.Add(CommonMethods.buildUInt32Parm("adnbrparm", Convert.ToInt32(myAddNbr), ParameterDirection.Input));
            CommonMethods.executeNonQuery("usprecordviews", parms, svm.SessionID);
            Response.Write("Ok");
            Response.Write(FD);
